#!/bin/bash
# factory/refa/flashall.sh -- refa wired factory flash (ch.09 S10 runbook executed by software).
...
if [ "${1:-plan}" = "--plan" ] || [ "${1:-plan}" = "plan" ]; then
  ...
  echo "mismatch guard: SKU match vs flashmap, mismatch = big-red-halt, never flash REF-A images into REF-B"
